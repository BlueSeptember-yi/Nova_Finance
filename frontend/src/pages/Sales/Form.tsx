...
const SalesForm = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    for (const line of validLines) {
      ...
      if (!product) {
        message.error(`商品"${line.product_name}"不存在`);
        return;
      }
      if (!line.unit_price || line.unit_price <= 0) {
        message.error(`商品"${line.product_name}"的售价必须大于0，请填写售价`);
        return;
      }
    }
    ...
    try {
      ...
      const response = await orderApi.createSales({
        ...
        return (
          <Select
            ...
            onChange={(value) => {
              ...
              if (product) {
                // 一次性更新所有字段
                // 售价可以从商品价格作为参考值，但允许用户修改
                const newLines = [...lines];
                newLines[index] = {
                  ...newLines[index],
                  product_id: value,
                  product_name: product.name,
                  unit_price: product.price ? Number(product.price) : 0, // 如果有价格则填充，否则为0让用户填写
                };
                setLines(newLines);
              }
            }}
          ...
          >
            {products.map(product => (
              <Select.Option key={product.product_id} value={product.product_id}>
                {`${product.name} [${product.sku}] ${product.price ? `(¥${product.price})` : '(无单价)'}`}
              </Select.Option>
            ))}
          </Select>
        );
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              >
                {customers.map(c => (
                  <Select.Option key={c.customer_id} value={c.customer_id}>
                    {c.name} {c.credit_limit > 0 && `(信用额度: ¥${formatMoney(c.credit_limit)})`}
                  </Select.Option>
                ))}
              </Select>
            </Form.Item>
            ...
            <Form.Item
              ...
              extra={
                form.getFieldValue('payment_method') === 'Credit' && form.getFieldValue('customer_id') ? (
                  (() => {
                    const selectedCustomer = customers.find(c => c.customer_id === form.getFieldValue('customer_id'));
                    if (selectedCustomer) {
                      if (selectedCustomer.credit_limit <= 0) {
                        return <span style={{ color: '#ff4d4f' }}>该客户未设置信用额度，不能使用赊销</span>;
                      }
                      return <span style={{ color: '#1890ff' }}>信用额度: ¥{formatMoney(selectedCustomer.credit_limit)}（系统会在创建和过账时检查可用额度）</span>;
                    }
                    return null;
                  })()
                ) : undefined
            ...
            >
              <Select>
                ...
                <Select.Option value="BankTransfer">银行转账</Select.Option>
                <Select.Option value="Credit">赊销</Select.Option>
              </Select>
            </Form.Item>
              ...
              <Table.Summary fixed>
                <Table.Summary.Row>
                  ...
                  <Table.Summary.Cell index={4}>
                    <span style={{ fontSize: 18, fontWeight: 'bold', color: '#eb2f96' }}>
